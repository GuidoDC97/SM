...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        public LayerMask groundMask = 9;//-1; //layermask of ground or other objects that affect height
        ...
        private void MyHeightCalculation()
        {
            if (useScrollwheelZooming)
            {
                float yDes = 0f;
                ...
                if (ScrollWheel > 0)
                {
                    yDes = transform.position.y - (0.1f) * scrollWheelZoomingSensitivity;
                    yDes = Mathf.Clamp(yDes, minHeight, maxHeight);

                    m_Transform.position = new Vector3(transform.position.x, yDes, transform.position.z);
                }
                if (ScrollWheel < 0)
                {
                    yDes = transform.position.y + (0.1f) * scrollWheelZoomingSensitivity;
        ...
        private void Rotation()
        {
            ...
            if (useMouseRotation && Input.GetKey(mouseRotationKey))
            {
                ...
                //my line
                //m_Transform.Rotate(Vector3.right, MouseAxis.y * Time.deltaTime * mouseRotationSpeed, Space.World);
            }
        ...
        private void LimitPosition()
        {
            ...
            */
            ...
            m_Transform.position = new Vector3(Mathf.Clamp(m_Transform.position.x, -50, 50),
                m_Transform.position.y,
                Mathf.Clamp(m_Transform.position.z, -50, 50));
        }
        ...
        public void ResetCamera()
        {
            ...
            m_Transform.rotation = Quaternion.Slerp(transform.rotation, startRotation, Time.time * 1.0f);

            //m_Transform.Rotate(startRotation.x - m_Transform.rotation.x, startRotation.y - m_Transform.rotation.y,
                //startRotation.z - m_Transform.rotation.z, Space.World);

            //m_Transform.rotation = startRotation;
        }
